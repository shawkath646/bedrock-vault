...
export class EncryptionRecordService {
    private logger: LoggerService;
    private recordFilePath: string;

    constructor(logger: LoggerService) {
        this.logger = logger;
        this.recordFilePath = path.join(app.getPath('userData'), RECORD_FILENAME);
    }
    ...
    public async getRecords(): Promise<EncryptionRecord[]> {
      try {
        const rawContent = await fs.readFile(this.recordFilePath, 'utf-8');
        ...
        if (nodeError.code !== 'ENOENT') {
          await this.logger.error('RecordUtil', `Failed to read records from record.json: ${error}`);
        }
      }
      return [];
    }

    public async saveRecord(record: EncryptionRecord): Promise<void> {
      try {
        ...
        await fs.mkdir(path.dirname(this.recordFilePath), { recursive: true });
        const tempFilePath = `${this.recordFilePath}.tmp`;
        await fs.writeFile(tempFilePath, JSON.stringify(recordsToSave, null, 2), 'utf-8');
        await fs.rename(tempFilePath, this.recordFilePath);
        await this.logger.info('RecordUtil', `Successfully saved encryption record for ${record.chunkName}`);
      } catch (error) {
        await this.logger.error('RecordUtil', `Failed to save encryption record: ${error}`);
      }
    }
    ...
    public async removeRecord(
      ...
      try {
        ...
        await fs.mkdir(path.dirname(this.recordFilePath), { recursive: true });
        const tempFilePath = `${this.recordFilePath}.tmp`;
        await fs.writeFile(tempFilePath, JSON.stringify(updatedRecords, null, 2), 'utf-8');
        await fs.rename(tempFilePath, this.recordFilePath);
        await this.logger.info('RecordUtil', `Successfully removed record for path: ${directoryPath}`);
        ...
        if (deletePermanently) {
          try {
            ...
            await this.logger.info('RecordUtil', `Moved directory to trash: ${directoryPath}`);
          } catch {
            await this.logger.info('RecordUtil', `Directory not found, skipping trash operation: ${directoryPath}`);
          }
        }
      } catch (error) {
        await this.logger.error('RecordUtil', `Failed to remove record for path ${directoryPath}: ${error}`);
        throw error;
      }
    }
}