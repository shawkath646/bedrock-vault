...
export function StagesViewer({ stages }: { stages: RenderEncryptionStages }) {
    const itemAnimation = {
    ...
    const isTerminalStage = (type: RenderEncryptionStages["current"]["type"]) => ["COMPLETED", "ABORT"].includes(type);
                ...
                <AnimatePresence mode="popLayout">
                    {stages?.completed.map((stage, idx) => (
                        ...
                        >
                            ...
                            {/* Stage Badge */}
                            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-sm text-xs whitespace-nowrap transition-colors duration-300 ${
                                stage.type === "FAILED" || stage.type === "ABORT" 
                    ...
                    {stages?.current && (
                        ...
                        >
                            ...
                            <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border shadow-sm text-xs whitespace-nowrap transition-all duration-300 ${
                                stages.current.type === "FAILED" || stages.current.type === "ABORT"
                                    ? "bg-destructive/10 border-destructive/20 text-destructive"
                                    : stages.current.type === "COMPLETED"
                                        ? "bg-primary/10 border-primary/20 text-primary"
                                        : stages.current.type === "WARNING"
                                            ? "bg-amber-500/10 border-amber-500/20 text-amber-500 dark:text-amber-400"
